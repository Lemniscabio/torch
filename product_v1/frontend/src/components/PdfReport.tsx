"use client";

import {
  Document,
  Page,
  Text,
  View,
  StyleSheet,
  Svg,
  Polygon,
  Circle,
  Line,
} from "@react-pdf/renderer";
import type { ProcessInputs, DerivedParameters, RiskScore, Confidence, RiskDomain, FeedingFrequency } from "@/lib/types";
import type { PartialAssessmentResult } from "@/lib/engine";
import {
  RISK_COLOURS,
  METABOLIC_HEAT_COEFF_KJ_PER_MMOL,
  RUSZKOWSKI_CONSTANT,
  RHO,
  KLA_CO2_O2_RATIO,
  H_CO2,
  G,
  ATMOSPHERIC_PRESSURE_PA,
  ATMOSPHERIC_PRESSURE_BAR,
  Y_CO2_INLET,
  MAX_CO2_MOLE_FRACTION,
  molarVolumeIdealGas,
  CO2_OUR_THRESHOLD,
  VANT_RIET_COALESCING_COEFFICIENT,
  VANT_RIET_COALESCING_PV_EXPONENT,
  VANT_RIET_COALESCING_VS_EXPONENT,
  FEEDING_FREQUENCY_SECONDS,
  FEEDING_FREQUENCY_LABELS,
} from "@/lib/constants";
import { deriveVesselGeometry, deriveGasVelocity, buildReactorScaleConfigs, resolveRq } from "@/lib/engine";
import { computeMixingTimeEnsemble } from "@/lib/engine/mixing/mixing_correlations";
import { deriveOxygenSolubility } from "@/lib/engine/derivations";
import type { ReactorScaleConfig } from "@/lib/engine/reactor_configs";
import { runHeatCapacityCheck } from "@/lib/engine/heat/heat_capacity";

// --- Constants ---

const NAVY = "#1B3A5C";
const TEAL = "#0D7377";
const FOOTER_TEXT =
  "Generated by Lemnisca Fermentation Scale-Up Risk Predictor \u2014 lemnisca.com";

const SPECIES_LABELS: Record<string, string> = {
  e_coli: "E. coli",
  b_subtilis: "B. subtilis",
  s_cerevisiae: "S. cerevisiae",
  p_pastoris: "P. pastoris",
  other_bacteria: "Other bacterium",
  other_yeast: "Other yeast",
};

const DOMAIN_LABELS: Record<string, string> = {
  otr: "Oxygen Transfer (OTR)",
  mixing: "Mixing",
  shear: "Shear Stress",
  co2: "CO\u2082 Accumulation",
  heat: "Heat Removal",
};

// --- Helpers ---

function fmt(n: number, decimals = 1): string {
  return n.toFixed(decimals);
}

function fmtInt(n: number): string {
  return Math.round(n).toString();
}

function roundTo5(n: number): number {
  return Math.round(n / 5) * 5;
}

function fmtRange95(value: number, std: number): string {
  const lo = roundTo5(value - 1.96 * std);
  const hi = roundTo5(value + 1.96 * std);
  return `${lo}–${hi === lo ? lo + 5 : hi}`;
}

function riskLabel(score: RiskScore): string {
  return score.charAt(0).toUpperCase() + score.slice(1);
}

function confidenceLabel(c: Confidence): string {
  switch (c) {
    case "high_confidence":
      return "High-confidence";
    case "reliable":
      return "Reliable";
    case "directional":
      return "Directional";
  }
}

function klaVantRiet(pv: number, vs: number): number {
  const kla_s =
    VANT_RIET_COALESCING_COEFFICIENT *
    Math.pow(pv, VANT_RIET_COALESCING_PV_EXPONENT) *
    Math.pow(vs, VANT_RIET_COALESCING_VS_EXPONENT);
  return kla_s * 3600;
}

function pilotVolume(vLab: number, vTarget: number): number {
  return Math.sqrt(vLab * vTarget);
}

function countEstimatedParams(inputs: ProcessInputs): {
  provided: number;
  estimated: number;
  total: number;
} {
  let provided = 0;
  let estimated = 0;

  // Always provided
  const alwaysProvided: (keyof ProcessInputs)[] = [
    "organism_class",
    "organism_species",
    "v_lab",
    "v_target",
    "rpm",
    "biomass_cdw_g_l",
  ];
  provided += alwaysProvided.length;

  // Conditionally provided vs estimated
  if (inputs.our_mode === "measured" && inputs.our_measured != null) {
    provided += 1;
  } else if (inputs.our_mode === "exhaust_gas") {
    provided += 1;
  } else {
    estimated += 1; // OUR estimated from literature
  }

  // Optional params that have defaults
  const optionalWithDefaults: { key: keyof ProcessInputs; defaultCheck: boolean }[] = [
    { key: "h_d_lab", defaultCheck: inputs.h_d_lab === 1.2 },
    { key: "h_d_target", defaultCheck: false },
    { key: "n_impellers", defaultCheck: false },
    { key: "impeller_type", defaultCheck: inputs.impeller_type === "rushton" },
    { key: "vvm", defaultCheck: inputs.vvm === 1.0 },
    { key: "do_setpoint", defaultCheck: inputs.do_setpoint === 30 },
    { key: "temperature", defaultCheck: false },
    { key: "t_cw_inlet", defaultCheck: inputs.t_cw_inlet === 12 },
  ];

  for (const p of optionalWithDefaults) {
    provided += 1; // All are user-provided or defaulted
  }

  const total = provided + estimated;
  return { provided, estimated, total };
}

function getHighImpactAssumptions(
  inputs: ProcessInputs,
  derived: DerivedParameters,
  results: PartialAssessmentResult,
): string[] {
  const assumptions: string[] = [];

  if (inputs.our_mode === "estimate") {
    assumptions.push(
      `OUR estimated from literature midpoint for ${SPECIES_LABELS[inputs.organism_species] ?? inputs.organism_species}. ` +
        `Actual OUR may range from ${fmt(derived.our_min ?? 0)} to ${fmt(derived.our_max ?? 0)} mmol/L/h. ` +
        `Higher OUR increases OTR, heat, and CO\u2082 risk; lower OUR decreases them.`,
    );
  }

  assumptions.push(
    `Van\u2019t Riet coalescing correlation assumed. Non-coalescing media would yield ` +
      `higher kLa at the same P/V, reducing OTR risk.`,
  );

  if (inputs.impeller_type === "unknown") {
    assumptions.push(
      `Unknown impeller type defaulted to Rushton constants (Np = 5.0, Pg/P = 0.60). ` +
        `Actual power draw may differ, affecting all scale-up projections.`,
    );
  }

  return assumptions.slice(0, 3);
}

function getSingleMeasurementAdvice(inputs: ProcessInputs): string {
  if (inputs.our_mode === "estimate") {
    return "A direct OUR measurement at peak biomass would most improve this assessment.";
  }
  return "A kLa characterisation of the target vessel at operating conditions would most improve this assessment.";
}

// --- Mitigation text (reused from dashboard logic) ---

function getMitigation(
  domain: string,
  results: PartialAssessmentResult,
  inputs: ProcessInputs,
): string | null {
  const { otr, mixing, shear, co2, heat } = results;
  const score =
    domain === "otr" ? otr.score :
    domain === "mixing" ? mixing.score :
    domain === "shear" ? shear.score :
    domain === "co2" ? co2.score :
    heat.score;

  if (score !== "high" && score !== "critical") return null;

  switch (domain) {
    case "otr":
      return `Required kLa of ${fmt(otr.kla_required)} h\u207B\u00B9 exceeds standard P/V achievability at ${fmtInt(inputs.v_target)} L. Options: (1) Reduce target biomass ~20% to bring OUR within standard kLa range. (2) Dual Rushton or Rushton-to-PBT cascade configuration. (3) Pressure overlay (+0.5 bar increases C* by ~15%).`;
    case "mixing":
      return `Mixing time at ${fmtInt(inputs.v_target)} L is ~${fmt(mixing.theta_mix_target)} s. Process mixing margin (tau_process/t_mix) = ${fmt(mixing.process_mixing_ratio_target ?? 0, 3)}. Options: (1) Increase agitation/mixing efficiency. (2) Reduce peak OUR demand. (3) Increase O2 availability at the same DO control strategy.`;
    case "shear":
      return `Tip speed of ${fmt(shear.tip_speed)} m/s at constant P/V exceeds ${SPECIES_LABELS[inputs.organism_species] ?? inputs.organism_species} threshold of ${fmt(shear.tip_speed_threshold)} m/s. Options: (1) Switch to pitched blade turbine. (2) Accept lower P/V with supplementary O\u2082 strategy. (3) Consider scale-up at constant tip speed instead of constant P/V.`;
    case "co2":
      return `Estimated pCO\u2082 at vessel bottom is ${fmt(co2.pco2_bottom ?? 0, 2)} bar. Options: (1) Increase sparging rate to enhance CO\u2082 stripping. (2) Reduce biomass target. (3) Lower H/D vessel geometry.`;
    case "heat":
      return `Metabolic heat generation of ${fmt(heat.q_metabolic)} kW exceeds jacket cooling capacity of ${fmt(heat.q_cool_max)} kW. Options: (1) Add internal cooling coil. (2) Reduce target biomass or OUR. (3) Lower process temperature if organism permits.`;
    default:
      return null;
  }
}

// --- Pilot computation (mirrors dashboard logic) ---

interface PilotRow {
  klaAchievable: number;
  mixingTime: number;
  o2_mixing_ratio: number;
  tipSpeed: number;
  pco2Bottom: number | null;
  heatKwM3: number;
}

function computePilot(inputs: ProcessInputs, derived: DerivedParameters): PilotRow {
  const vPilot = pilotVolume(inputs.v_lab, inputs.v_target);
  const geometry = deriveVesselGeometry(vPilot, inputs.h_d_target, inputs.impeller_type, inputs.dt_ratio_target);
  const gas = deriveGasVelocity(inputs.vvm, vPilot, geometry.a_cross);

  const klaAchievable = klaVantRiet(derived.pv_lab, gas.vs);

  const pilotT = geometry.t_diameter;
  const pilotDimp = geometry.d_imp;
  const epsilon = derived.pv_lab / RHO;
  const mixingTime =
    RUSZKOWSKI_CONSTANT * pilotT * pilotT /
    (Math.pow(epsilon, 1 / 3) * Math.pow(pilotDimp, 4 / 3));

  const cStarPilot = deriveOxygenSolubility(
    inputs.temperature,
    inputs.do_setpoint,
    geometry.h_liquid,
    inputs.o2_inlet ?? 20.9,
  ).c_star_avg;

  const t_feed_pilot = inputs.process_type === "fed_batch" && inputs.feeding_frequency
    ? FEEDING_FREQUENCY_SECONDS[inputs.feeding_frequency as FeedingFrequency]
    : Infinity;
  const t_o2_pilot   = derived.our_peak > 0 ? 3600 * Math.max(cStarPilot, 1e-9) / derived.our_peak : Infinity;
  const t_process_pilot = Math.min(t_feed_pilot, t_o2_pilot);

  const o2_mixing_ratio = derived.our_peak > 0
    ? t_process_pilot / Math.max(mixingTime, 1e-9)
    : 0;

  const dImpLab = derived.lab_geometry.d_imp;
  const nTarget = derived.n_rps * Math.pow(dImpLab / pilotDimp, 5 / 3);
  const tipSpeed = Math.PI * nTarget * pilotDimp;

  let pco2Bottom: number | null = null;
  const activated = derived.our_peak > CO2_OUR_THRESHOLD;
  if (activated) {
    const cer = resolveRq(inputs.organism_species) * derived.our_peak;
    const klaO2Pilot = klaVantRiet(derived.pv_lab, gas.vs);
    const klaCo2 = KLA_CO2_O2_RATIO * klaO2Pilot;
    const pco2BulkAtm = (cer / 1000) / (klaCo2 * H_CO2);
    const pco2Bulk = pco2BulkAtm * ATMOSPHERIC_PRESSURE_BAR;
    const dpHydro = RHO * G * geometry.h_liquid;
    pco2Bottom = pco2Bulk + (dpHydro / ATMOSPHERIC_PRESSURE_PA) * ATMOSPHERIC_PRESSURE_BAR;
  }

  const vPilotM3 = geometry.volume_m3;
  const qMetabolic = METABOLIC_HEAT_COEFF_KJ_PER_MMOL[inputs.organism_species] * derived.our_peak * vPilotM3;
  const heatKwM3 = vPilotM3 > 0 ? qMetabolic / vPilotM3 : 0;

  return { klaAchievable, mixingTime, o2_mixing_ratio, tipSpeed, pco2Bottom, heatKwM3 };
}

// --- Styles ---

const s = StyleSheet.create({
  page: {
    paddingTop: 40,
    paddingBottom: 60,
    paddingHorizontal: 40,
    fontFamily: "Helvetica",
    fontSize: 9,
    color: "#333333",
  },
  footer: {
    position: "absolute",
    bottom: 25,
    left: 40,
    right: 40,
    fontSize: 7,
    color: "#999999",
    textAlign: "center",
  },
  // Header
  headerBar: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    borderBottomWidth: 2,
    borderBottomColor: NAVY,
    paddingBottom: 8,
    marginBottom: 16,
  },
  logoText: {
    fontSize: 16,
    fontFamily: "Helvetica-Bold",
    color: NAVY,
  },
  dateText: {
    fontSize: 8,
    color: "#666666",
  },
  // Section titles
  sectionTitle: {
    fontSize: 12,
    fontFamily: "Helvetica-Bold",
    color: NAVY,
    marginBottom: 8,
    marginTop: 12,
  },
  subsectionTitle: {
    fontSize: 10,
    fontFamily: "Helvetica-Bold",
    color: TEAL,
    marginBottom: 4,
    marginTop: 8,
  },
  // Parameter summary box
  paramBox: {
    backgroundColor: "#F0F4F8",
    borderRadius: 4,
    padding: 10,
    marginBottom: 12,
  },
  paramRow: {
    flexDirection: "row",
    marginBottom: 2,
  },
  paramLabel: {
    width: 120,
    fontSize: 8,
    color: "#555555",
    fontFamily: "Helvetica-Bold",
  },
  paramValue: {
    fontSize: 8,
    color: "#333333",
  },
  // Bottleneck
  bottleneckBox: {
    backgroundColor: "#FFF4E5",
    borderLeftWidth: 3,
    borderLeftColor: "#E67E22",
    padding: 10,
    marginBottom: 12,
  },
  bottleneckText: {
    fontSize: 11,
    fontFamily: "Helvetica-Bold",
    color: "#333333",
    marginBottom: 4,
  },
  bottleneckSub: {
    fontSize: 8,
    color: "#555555",
  },
  // Risk score table
  riskTable: {
    marginBottom: 12,
  },
  riskTableHeader: {
    flexDirection: "row",
    backgroundColor: NAVY,
    paddingVertical: 4,
    paddingHorizontal: 6,
  },
  riskTableHeaderCell: {
    fontSize: 7,
    fontFamily: "Helvetica-Bold",
    color: "#FFFFFF",
  },
  riskTableRow: {
    flexDirection: "row",
    paddingVertical: 4,
    paddingHorizontal: 6,
    borderBottomWidth: 0.5,
    borderBottomColor: "#E0E0E0",
  },
  riskTableRowAlt: {
    flexDirection: "row",
    paddingVertical: 4,
    paddingHorizontal: 6,
    borderBottomWidth: 0.5,
    borderBottomColor: "#E0E0E0",
    backgroundColor: "#F8F9FA",
  },
  riskDomainCol: { width: "25%" },
  riskScoreCol: { width: "15%", textAlign: "center" },
  riskKeyCol: { width: "25%", textAlign: "right" },
  riskConfCol: { width: "35%", textAlign: "right" },
  scoreBadge: {
    fontSize: 7,
    fontFamily: "Helvetica-Bold",
    color: "#FFFFFF",
    paddingHorizontal: 4,
    paddingVertical: 1,
    borderRadius: 2,
  },
  bodyText: {
    fontSize: 8,
    color: "#444444",
    lineHeight: 1.5,
    marginBottom: 6,
  },
  // Detail sections
  domainSection: {
    marginBottom: 10,
    borderBottomWidth: 0.5,
    borderBottomColor: "#E0E0E0",
    paddingBottom: 8,
  },
  domainHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    marginBottom: 4,
  },
  domainName: {
    fontSize: 10,
    fontFamily: "Helvetica-Bold",
    color: NAVY,
  },
  paramTable: {
    marginTop: 4,
    marginBottom: 4,
  },
  paramTableRow: {
    flexDirection: "row",
    paddingVertical: 2,
    borderBottomWidth: 0.5,
    borderBottomColor: "#F0F0F0",
  },
  paramTableLabel: {
    width: "55%",
    fontSize: 8,
    color: "#555555",
  },
  paramTableValue: {
    width: "45%",
    fontSize: 8,
    color: "#333333",
    textAlign: "right",
    fontFamily: "Courier",
  },
  mitigationBox: {
    backgroundColor: "#FFF8F0",
    borderLeftWidth: 2,
    borderLeftColor: "#E67E22",
    padding: 6,
    marginTop: 4,
  },
  mitigationLabel: {
    fontSize: 7,
    fontFamily: "Helvetica-Bold",
    color: "#999999",
    textTransform: "uppercase",
    marginBottom: 2,
  },
  mitigationText: {
    fontSize: 8,
    color: "#444444",
    lineHeight: 1.4,
  },
  // Projections table
  projTable: {
    marginBottom: 12,
  },
  projHeader: {
    flexDirection: "row",
    backgroundColor: NAVY,
    paddingVertical: 5,
    paddingHorizontal: 6,
  },
  projHeaderCell: {
    fontSize: 7,
    fontFamily: "Helvetica-Bold",
    color: "#FFFFFF",
  },
  projRow: {
    flexDirection: "row",
    paddingVertical: 4,
    paddingHorizontal: 6,
    borderBottomWidth: 0.5,
    borderBottomColor: "#E0E0E0",
  },
  projRowAlt: {
    flexDirection: "row",
    paddingVertical: 4,
    paddingHorizontal: 6,
    borderBottomWidth: 0.5,
    borderBottomColor: "#E0E0E0",
    backgroundColor: "#F8F9FA",
  },
  projParamCol: { width: "34%" },
  projValCol: { width: "22%", textAlign: "right", fontFamily: "Courier" },
  // Reliability
  reliabilityBox: {
    backgroundColor: "#F0F4F8",
    borderRadius: 4,
    padding: 10,
    marginBottom: 10,
  },
  scopeBox: {
    backgroundColor: "#FFF4E5",
    borderLeftWidth: 3,
    borderLeftColor: TEAL,
    padding: 10,
    marginTop: 8,
  },
  scopeText: {
    fontSize: 8,
    color: "#555555",
    lineHeight: 1.5,
    fontStyle: "italic",
  },
});

// --- Risk score colour indicator (text-based) ---

function ScoreBadge({ score }: { score: RiskScore }) {
  return (
    <Text style={[s.scoreBadge, { backgroundColor: RISK_COLOURS[score] }]}>
      {riskLabel(score)}
    </Text>
  );
}

// --- Footer component ---

function PageFooter() {
  return <Text style={s.footer}>{FOOTER_TEXT}</Text>;
}

// --- Header component ---

function PageHeader() {
  const now = new Date();
  const dateStr = now.toLocaleDateString("en-GB", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  return (
    <View style={s.headerBar}>
      <Text style={s.logoText}>Lemnisca</Text>
      <Text style={s.dateText}>{dateStr}</Text>
    </View>
  );
}

// --- Page 1: Executive Summary ---

function ExecutiveSummary({
  inputs,
  derived,
  results,
}: {
  inputs: ProcessInputs;
  derived: DerivedParameters;
  results: PartialAssessmentResult;
}) {
  const { otr, mixing, shear, co2, heat, primary_bottleneck: bottleneck } = results;
  const scaleRatio = inputs.v_target / inputs.v_lab;
  const species = SPECIES_LABELS[inputs.organism_species] ?? inputs.organism_species;

  // Build summary paragraph
  const domains = [
    { name: "OTR", score: otr.score },
    { name: "Mixing", score: mixing.score },
    { name: "Shear", score: shear.score },
    { name: "CO\u2082", score: co2.score },
    { name: "Heat", score: heat.score },
  ];
  const highCrit = domains.filter(
    (d) => d.score === "high" || d.score === "critical",
  );
  const allLow = domains.every((d) => d.score === "low");

  let summaryParagraph: string;
  if (allLow) {
    summaryParagraph = `Scale-up from ${fmtInt(inputs.v_lab)} L to ${fmtInt(inputs.v_target)} L presents low risk across all five domains. Standard equipment and operating procedures are expected to maintain process performance at the target scale.`;
  } else if (highCrit.length === 0) {
    summaryParagraph = `Scale-up from ${fmtInt(inputs.v_lab)} L to ${fmtInt(inputs.v_target)} L presents moderate challenges. No critical constraints were identified. Standard scale-up procedures with attention to the flagged domains should be sufficient.`;
  } else {
    const names = highCrit.map((d) => d.name).join(", ");
    summaryParagraph = `Scale-up from ${fmtInt(inputs.v_lab)} L to ${fmtInt(inputs.v_target)} L presents ${highCrit.length === 1 ? "a significant" : "significant"} constraint${highCrit.length > 1 ? "s" : ""} in ${names}. ${bottleneck.statement} Targeted engineering intervention is recommended before proceeding to pilot scale.`;
  }

  const riskRows = [
    { domain: "Oxygen Transfer (OTR)", score: otr.score, key: `OTR/OUR: ${fmt(otr.kla_ratio, 2)}`, conf: confidenceLabel(otr.confidence) },
    { domain: "Mixing", score: mixing.score, key: `θ_mix: ${fmt(mixing.theta_mix_target)} s, process margin: ${fmt(mixing.process_mixing_ratio_target ?? 0, 3)}`, conf: confidenceLabel(mixing.confidence) },
    { domain: "Shear Stress", score: shear.score, key: `Tip speed: ${fmt(shear.tip_speed)} m/s`, conf: confidenceLabel(shear.confidence) },
    { domain: "CO\u2082 Accumulation", score: co2.score, key: co2.activated && co2.pco2_bottom != null ? `pCO\u2082: ${fmt(co2.pco2_bottom, 3)} bar` : "Not activated", conf: confidenceLabel(co2.confidence) },
    { domain: "Heat Removal", score: heat.score, key: `Q_cool/Q_metabolic: ${fmt(heat.heat_transfer_margin ?? 0, 2)}`, conf: confidenceLabel(heat.confidence) },
  ];

  return (
    <Page size="A4" style={s.page}>
      <PageHeader />

      <Text style={s.sectionTitle}>Executive Summary</Text>

      {/* Process parameters */}
      <View style={s.paramBox}>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Organism</Text>
          <Text style={s.paramValue}>{species}</Text>
        </View>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Lab volume</Text>
          <Text style={s.paramValue}>{fmtInt(inputs.v_lab)} L</Text>
        </View>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Target volume</Text>
          <Text style={s.paramValue}>{fmtInt(inputs.v_target)} L</Text>
        </View>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Scale ratio</Text>
          <Text style={s.paramValue}>{fmt(scaleRatio, 0)}\u00D7</Text>
        </View>
      </View>

      {/* Primary bottleneck */}
      <View style={s.bottleneckBox}>
        <Text style={s.bottleneckText}>{bottleneck.statement}</Text>
      </View>

      {/* Risk score table */}
      <View style={s.riskTable}>
        <View style={s.riskTableHeader}>
          <Text style={[s.riskTableHeaderCell, s.riskDomainCol]}>Domain</Text>
          <Text style={[s.riskTableHeaderCell, s.riskScoreCol]}>Score</Text>
          <Text style={[s.riskTableHeaderCell, s.riskKeyCol]}>Key Metric</Text>
          <Text style={[s.riskTableHeaderCell, s.riskConfCol]}>Confidence</Text>
        </View>
        {riskRows.map((row, i) => (
          <View key={i} style={i % 2 === 0 ? s.riskTableRow : s.riskTableRowAlt}>
            <Text style={[{ fontSize: 8 }, s.riskDomainCol]}>{row.domain}</Text>
            <View style={[s.riskScoreCol, { alignItems: "center" }]}>
              <ScoreBadge score={row.score} />
            </View>
            <Text style={[{ fontSize: 8, fontFamily: "Courier" }, s.riskKeyCol]}>{row.key}</Text>
            <Text style={[{ fontSize: 7, color: "#666666" }, s.riskConfCol]}>{row.conf}</Text>
          </View>
        ))}
      </View>

      {/* Summary paragraph */}
      <Text style={s.bodyText}>{summaryParagraph}</Text>

      <PageFooter />
    </Page>
  );
}

// --- Page 2: Risk Assessment Detail ---

function RiskDetail({
  inputs,
  derived,
  results,
}: {
  inputs: ProcessInputs;
  derived: DerivedParameters;
  results: PartialAssessmentResult;
}) {
  const { otr, mixing, shear, co2, heat } = results;

  const domainSections: {
    id: string;
    name: string;
    score: RiskScore;
    confidence: Confidence;
    driver: string;
    params: { label: string; value: string }[];
    thresholdNote: string;
    uncertaintyNote?: string;
  }[] = [
    {
      id: "otr",
      name: "Oxygen Transfer (OTR)",
      score: otr.score,
      confidence: otr.confidence,
      driver: otr.driver,
      params: [
        { label: "OUR peak", value: `${fmt(derived.our_peak)} mmol/L/h` },
        { label: "kLa required", value: `${fmt(otr.kla_required)} h\u207B\u00B9` },
        { label: "kLa achievable (moderate P/V)", value: otr.kla_target_std ? `${fmtRange95(otr.kla_target_moderate, otr.kla_target_std)} h⁻¹` : `${roundTo5(otr.kla_target_moderate)} h⁻¹` },
        { label: "OTR/OUR ratio", value: fmt(otr.kla_ratio, 2) },
        { label: "P/V at target", value: `${fmt(otr.pv_target)} W/m\u00B3` },
      ],
      thresholdNote: ">1.5 Low, 1.0\u20131.5 Moderate, 0.7\u20131.0 High, <0.7 Critical",
    },
    {
      id: "mixing",
      name: "Mixing",
      score: mixing.score,
      confidence: mixing.confidence,
      driver: mixing.driver,
      params: [
        { label: "θ_mix (lab)", value: `${fmtRange95(mixing.theta_mix_lab, mixing.theta_mix_lab_std)} s` },
        { label: "θ_mix (target)", value: `${fmtRange95(mixing.theta_mix_target, mixing.theta_mix_target_std)} s` },
        { label: "C_L (lab)", value: `${fmt(derived.c_l_lab, 3)} mmol/L` },
        { label: "C_L (target)", value: `${fmt(derived.c_l, 3)} mmol/L` },
        { label: "O2 depletion time (lab)", value: Number.isFinite(mixing.oxygen_depletion_time_lab_s) ? `${fmt(mixing.oxygen_depletion_time_lab_s, 2)} s` : "∞" },
        { label: "O2 depletion time (target)", value: Number.isFinite(mixing.oxygen_depletion_time_target_s) ? `${fmt(mixing.oxygen_depletion_time_target_s, 2)} s` : "∞" },
        { label: "Process mixing margin (lab)", value: `${fmt(mixing.process_mixing_ratio_lab ?? 0, 3)} ± ${fmt(mixing.process_mixing_ratio_lab_std, 3)}` },
        { label: "Process mixing margin (target)", value: `${fmt(mixing.process_mixing_ratio_target ?? 0, 3)} ± ${fmt(mixing.process_mixing_ratio_target_std, 3)}` },
      ],
      thresholdNote: "O2 margin: >10.0 Low, 1.0\u201310.0 Moderate, 0.1\u20131.0 High, \u22640.1 Critical",
      uncertaintyNote: `Mixing time scales as (V_target/V_lab)^(1/3). Actual performance depends on vessel internals and impeller configuration.`,
    },
    {
      id: "shear",
      name: "Shear Stress",
      score: shear.score,
      confidence: shear.confidence,
      driver: shear.driver,
      params: [
        { label: "Tip speed (target)", value: `${fmt(shear.tip_speed)} m/s` },
        { label: "Organism threshold", value: `${fmt(shear.tip_speed_threshold)} m/s` },
        { label: "Tip speed ratio", value: fmt(shear.tip_speed_ratio, 2) },
      ],
      thresholdNote: "<0.7 Low, 0.7\u20131.0 Moderate, 1.0\u20131.3 High, >1.3 Critical",
      uncertaintyNote: `Based on constant P/V scaling. Constant tip speed scaling would yield different results.`,
    },
    {
      id: "co2",
      name: "CO\u2082 Accumulation",
      score: co2.score,
      confidence: co2.confidence,
      driver: co2.driver,
      params: co2.activated
        ? [
            { label: "CER", value: `${fmt(co2.cer ?? 0)} mmol/L/h` },
            { label: "kLa_CO\u2082", value: `${fmt(co2.kla_co2 ?? 0)} h\u207B\u00B9` },
            { label: "pCO\u2082 bulk", value: `${fmt(co2.pco2_bulk ?? 0, 3)} bar` },
            { label: "pCO\u2082 bottom", value: `${fmt(co2.pco2_bottom ?? 0, 3)} bar` },
          ]
        : [{ label: "Status", value: "Not activated (below biomass/OUR thresholds)" }],
      thresholdNote: "<0.08 bar Low, 0.08\u20130.15 Moderate, 0.15\u20130.25 High, >0.25 Critical",
      uncertaintyNote: `CO\u2082 carries the widest confidence interval of the five domains. Simplified mass balance model.`,
    },
    {
      id: "heat",
      name: "Heat Removal",
      score: heat.score,
      confidence: heat.confidence,
      driver: heat.driver,
      params: [
        { label: "Q_metabolic", value: `${fmt(heat.q_metabolic, 2)} kW` },
        { label: "Q_cool_max", value: `${fmt(heat.q_cool_max, 2)} kW` },
        { label: "Heat transfer margin Q_cool/Q_metabolic", value: fmt(heat.heat_transfer_margin ?? 0, 2) },
        { label: "A_jacket", value: `${fmt(heat.a_jacket, 2)} m\u00B2` },
        { label: "\u0394T_lm", value: `${fmt(heat.dt_lm, 1)}\u00B0C` },
      ],
      thresholdNote: ">1.67 Low, 1.18\u20131.67 Moderate, 1.00\u20131.18 High, \u22641.00 Critical",
      uncertaintyNote: `Assumes U = 400 W/m\u00B2\u00B7K and cooling water outlet T_cw_in + 10\u00B0C. Internal coils not modelled.`,
    },
  ];

  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Risk Assessment Detail</Text>

      {domainSections.map((section) => {
        const mitigation = getMitigation(section.id, results, inputs);
        return (
          <View key={section.id} style={s.domainSection} wrap={false}>
            <View style={s.domainHeader}>
              <Text style={s.domainName}>{section.name}</Text>
              <ScoreBadge score={section.score} />
            </View>

            {/* Key params */}
            <View style={s.paramTable}>
              {section.params.map((p, i) => (
                <View key={i} style={s.paramTableRow}>
                  <Text style={s.paramTableLabel}>{p.label}</Text>
                  <Text style={s.paramTableValue}>{p.value}</Text>
                </View>
              ))}
            </View>

            {/* Threshold */}
            <Text style={{ fontSize: 7, color: "#888888", marginBottom: 2 }}>
              Thresholds: {section.thresholdNote}
            </Text>

            {/* Uncertainty */}
            {section.uncertaintyNote && (
              <Text style={{ fontSize: 7, color: "#888888", marginBottom: 2 }}>
                Uncertainty: {section.uncertaintyNote}
              </Text>
            )}

            {/* Confidence */}
            <Text style={{ fontSize: 7, color: "#888888" }}>
              Confidence: {confidenceLabel(section.confidence)} \u2014 {section.driver}
            </Text>

            {/* Mitigation */}
            {mitigation && (
              <View style={s.mitigationBox}>
                <Text style={s.mitigationLabel}>Mitigation Recommendation</Text>
                <Text style={s.mitigationText}>{mitigation}</Text>
              </View>
            )}
          </View>
        );
      })}

      <PageFooter />
    </Page>
  );
}

// --- Page 3: Quantitative Projections ---

function Projections({
  inputs,
  derived,
  results,
}: {
  inputs: ProcessInputs;
  derived: DerivedParameters;
  results: PartialAssessmentResult;
}) {
  const { otr, mixing, shear, co2, heat } = results;
  const vPilot = pilotVolume(inputs.v_lab, inputs.v_target);
  const pilot = computePilot(inputs, derived);

  const labTipSpeed = Math.PI * derived.n_rps * derived.lab_geometry.d_imp;
  const heatKwM3Lab = METABOLIC_HEAT_COEFF_KJ_PER_MMOL[inputs.organism_species] * derived.our_peak;
  const heatKwM3Target =
    derived.target_geometry.volume_m3 > 0
      ? heat.q_metabolic / derived.target_geometry.volume_m3
      : 0;

  const rows: { param: string; lab: string; pilotVal: string; prod: string }[] = [
    {
      param: "kLa achievable (h\u207B\u00B9)",
      lab: otr.kla_lab_std ? fmtRange95(otr.kla_lab, otr.kla_lab_std) : String(roundTo5(otr.kla_lab)),
      pilotVal: String(roundTo5(pilot.klaAchievable)),
      prod: otr.kla_target_std ? fmtRange95(otr.kla_target_moderate, otr.kla_target_std) : String(roundTo5(otr.kla_target_moderate)),
    },
    {
      param: "kLa required (h\u207B\u00B9)",
      lab: fmt(otr.kla_required),
      pilotVal: fmt(otr.kla_required),
      prod: fmt(otr.kla_required),
    },
    {
      param: "Mixing time (s)",
      lab: fmtRange95(mixing.theta_mix_lab, mixing.theta_mix_lab_std),
      pilotVal: String(roundTo5(pilot.mixingTime)),
      prod: fmtRange95(mixing.theta_mix_target, mixing.theta_mix_target_std),
    },
    {
      param: "Process mixing margin (τ_process/τ_mix)",
      lab: `${fmt(mixing.process_mixing_ratio_lab ?? 0, 3)} ± ${fmt(mixing.process_mixing_ratio_lab_std, 3)}`,
      pilotVal: fmt(pilot.o2_mixing_ratio, 3),
      prod: `${fmt(mixing.process_mixing_ratio_target ?? 0, 3)} ± ${fmt(mixing.process_mixing_ratio_target_std, 3)}`,
    },
    {
      param: "Tip speed (m/s)",
      lab: fmt(labTipSpeed),
      pilotVal: fmt(pilot.tipSpeed),
      prod: fmt(shear.tip_speed),
    },
    {
      param: "pCO\u2082 bottom (bar)",
      lab: "\u2014",
      pilotVal: pilot.pco2Bottom != null ? fmt(pilot.pco2Bottom, 3) : "\u2014",
      prod: co2.pco2_bottom != null ? fmt(co2.pco2_bottom, 3) : "\u2014",
    },
    {
      param: "Metabolic heat (kW/m\u00B3)",
      lab: fmt(heatKwM3Lab, 2),
      pilotVal: fmt(pilot.heatKwM3, 2),
      prod: fmt(heatKwM3Target, 2),
    },
  ];

  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Quantitative Projections</Text>

      <Text style={s.bodyText}>
        Projections across three scales: lab ({fmtInt(inputs.v_lab)} L), pilot ({fmtInt(vPilot)} L, geometric mean), and production ({fmtInt(inputs.v_target)} L). All values computed at constant P/V (moderate scenario, 1.0\u00D7).
      </Text>

      <View style={s.projTable}>
        <View style={s.projHeader}>
          <Text style={[s.projHeaderCell, s.projParamCol]}>Parameter</Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>
            Lab ({fmtInt(inputs.v_lab)} L)
          </Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>
            Pilot ({fmtInt(vPilot)} L)
          </Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>
            Production ({fmtInt(inputs.v_target)} L)
          </Text>
        </View>
        {rows.map((row, i) => (
          <View key={i} style={i % 2 === 0 ? s.projRow : s.projRowAlt}>
            <Text style={[{ fontSize: 8 }, s.projParamCol]}>{row.param}</Text>
            <Text style={[{ fontSize: 8, fontFamily: "Courier" }, s.projValCol]}>
              {row.lab}
            </Text>
            <Text style={[{ fontSize: 8, fontFamily: "Courier" }, s.projValCol]}>
              {row.pilotVal}
            </Text>
            <Text style={[{ fontSize: 8, fontFamily: "Courier" }, s.projValCol]}>
              {row.prod}
            </Text>
          </View>
        ))}
      </View>

      {/* Additional context */}
      <Text style={s.subsectionTitle}>Scale-Up Basis</Text>
      <View style={s.paramTable}>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>Scale-up rule</Text>
          <Text style={s.paramTableValue}>Constant P/V</Text>
        </View>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>P/V (lab)</Text>
          <Text style={s.paramTableValue}>{fmt(derived.pv_lab)} W/m{"\u00B3"}</Text>
        </View>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>Impeller type</Text>
          <Text style={s.paramTableValue}>{inputs.impeller_type}</Text>
        </View>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>H/D (target)</Text>
          <Text style={s.paramTableValue}>{fmt(inputs.h_d_target, 1)}</Text>
        </View>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>Impellers (target)</Text>
          <Text style={s.paramTableValue}>{inputs.n_impellers}</Text>
        </View>
        <View style={s.paramTableRow}>
          <Text style={s.paramTableLabel}>VVM</Text>
          <Text style={s.paramTableValue}>{fmt(inputs.vvm, 1)}</Text>
        </View>
      </View>

      <PageFooter />
    </Page>
  );
}

// --- Page 4: Reliability Statement ---

function ReliabilityStatement({
  inputs,
  derived,
  results,
}: {
  inputs: ProcessInputs;
  derived: DerivedParameters;
  results: PartialAssessmentResult;
}) {
  const { provided, estimated, total } = countEstimatedParams(inputs);
  const assumptions = getHighImpactAssumptions(inputs, derived, results);
  const measurementAdvice = getSingleMeasurementAdvice(inputs);

  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Reliability Statement</Text>

      {/* Input provenance */}
      <Text style={s.subsectionTitle}>Input Provenance</Text>
      <View style={s.reliabilityBox}>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Parameters provided</Text>
          <Text style={s.paramValue}>{provided} of {total}</Text>
        </View>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>Parameters estimated</Text>
          <Text style={s.paramValue}>{estimated} of {total}</Text>
        </View>
        <View style={s.paramRow}>
          <Text style={s.paramLabel}>OUR source</Text>
          <Text style={s.paramValue}>
            {inputs.our_mode === "measured"
              ? "User-provided measurement"
              : inputs.our_mode === "exhaust_gas"
                ? "Derived from exhaust gas analysis"
                : "Estimated from literature"}
          </Text>
        </View>
      </View>

      {/* High-impact assumptions */}
      <Text style={s.subsectionTitle}>Highest-Impact Assumptions</Text>
      {assumptions.map((a, i) => (
        <View key={i} style={{ flexDirection: "row", marginBottom: 4 }}>
          <Text style={{ fontSize: 8, color: TEAL, marginRight: 4 }}>{i + 1}.</Text>
          <Text style={[s.bodyText, { flex: 1, marginBottom: 0 }]}>{a}</Text>
        </View>
      ))}

      {/* Single measurement */}
      <Text style={[s.subsectionTitle, { marginTop: 12 }]}>
        Most Valuable Additional Measurement
      </Text>
      <Text style={s.bodyText}>{measurementAdvice}</Text>

      {/* Scope limitation */}
      <View style={s.scopeBox}>
        <Text style={[s.subsectionTitle, { color: NAVY, marginTop: 0 }]}>
          Scope Limitation
        </Text>
        <Text style={s.scopeText}>
          This assessment is based on empirical correlations with {"\u00B1"}30{"\u2013"}40% uncertainty on kLa. It does not predict yield or replace pilot experimentation.
        </Text>
      </View>

      <PageFooter />
    </Page>
  );
}

// --- New baseline report helpers ---

const DISPLAY_DOMAIN_ORDER: RiskDomain[] = ["mixing", "otr", "shear", "co2", "heat"];

const SCALEUP_CRITERION_LABELS: Record<NonNullable<ProcessInputs["scaleup_criterion"]>, string> = {
  power_per_volume: "P/V",
  kla: "kLa",
  shear: "tip speed",
};

const CONFIDENCE_DOT: Record<Confidence, string> = {
  high_confidence: RISK_COLOURS.low,
  reliable: RISK_COLOURS.low,
  directional: RISK_COLOURS.moderate,
};

interface PdfProjectionScaleSummary {
  volumeL: number;
  rpm: number;
  aerationLpm: number;
  vvm: number;
  impellerDiameterM: number;
  reactorHeightM: number;
  klaMean: number;
  klaStd: number;
  mixingTimeS: number;
  mixingTimeStd: number;
  tipSpeedMS: number;
  pco2Bar: number | null;
  metabolicHeatKW: number;
  reactorHeatCapacityKW: number;
}

interface PdfScaleProjectionSummary {
  lab: PdfProjectionScaleSummary;
  pilot: PdfProjectionScaleSummary;
  production: PdfProjectionScaleSummary;
  pilotVolumeL: number;
}

const DOMAIN_SHORT_LABELS: Record<RiskDomain, string> = {
  mixing: "Mixing",
  otr: "OTR",
  shear: "Shear",
  co2: "CO2",
  heat: "Heat",
};

const DOMAIN_FULL_LABELS: Record<RiskDomain, string> = {
  mixing: "Mixing Time",
  otr: "Oxygen Transfer",
  shear: "Shear Stress",
  co2: "CO2 Accumulation",
  heat: "Heat Removal",
};


function scoreSeverity(score: RiskScore): number {
  return { low: 0, moderate: 1, high: 2, critical: 3 }[score];
}

function radarRadius(score: RiskScore): number {
  switch (score) {
    case "low":      return 0.25;
    case "moderate": return 0.5;
    case "high":     return 0.75;
    case "critical": return 1.0;
  }
}

const PDF_RISK_NUMERIC: Record<RiskScore, number> = { low: 1, moderate: 2, high: 3, critical: 4 };
const PDF_DOMAIN_ORDER: RiskDomain[] = ["mixing", "otr", "shear", "co2", "heat"];

function pdfHexToRgb(hex: string): [number, number, number] {
  return [parseInt(hex.slice(1, 3), 16), parseInt(hex.slice(3, 5), 16), parseInt(hex.slice(5, 7), 16)];
}

function pdfLerpRgb(a: [number, number, number], b: [number, number, number], t: number): string {
  const r = Math.round(a[0] + (b[0] - a[0]) * t);
  const g = Math.round(a[1] + (b[1] - a[1]) * t);
  const bl = Math.round(a[2] + (b[2] - a[2]) * t);
  return `rgb(${r},${g},${bl})`;
}

function pdfCompositeScoreColour(scores: Record<RiskDomain, RiskScore>): string {
  const total = PDF_DOMAIN_ORDER.reduce((sum, d) => sum + PDF_RISK_NUMERIC[scores[d]], 0);
  const t = Math.max(0, Math.min(1, (total - 5) / 15));
  const stops: Array<[number, number, number]> = [
    pdfHexToRgb(RISK_COLOURS.low),
    pdfHexToRgb(RISK_COLOURS.moderate),
    pdfHexToRgb(RISK_COLOURS.high),
    pdfHexToRgb(RISK_COLOURS.critical),
  ];
  const segment = Math.min(2, Math.floor(t * 3));
  return pdfLerpRgb(stops[segment], stops[segment + 1], t * 3 - segment);
}

const RISK_SCORE_LABELS: Record<RiskScore, string> = {
  low: "Low", moderate: "Moderate", high: "High", critical: "Critical",
};

function labScores(results: PartialAssessmentResult): Record<RiskDomain, RiskScore> {
  return {
    mixing: results.mixing.score_lab ?? results.mixing.score,
    otr: results.otr.score_lab ?? results.otr.score,
    shear: results.shear.score_lab ?? results.shear.score,
    co2: results.co2.lab?.score ?? results.co2.score,
    heat: results.heat.lab?.score ?? results.heat.score,
  };
}

function targetScores(results: PartialAssessmentResult): Record<RiskDomain, RiskScore> {
  return {
    mixing: results.mixing.score_target ?? results.mixing.score,
    otr: results.otr.score_target ?? results.otr.score,
    shear: results.shear.score_target ?? results.shear.score,
    co2: results.co2.target?.score ?? results.co2.score,
    heat: results.heat.target?.score ?? results.heat.score,
  };
}

function scoreValue(domain: RiskDomain, results: PartialAssessmentResult, scale: "lab" | "target"): string {
  const { otr, mixing, shear, co2, heat } = results;
  const isLab = scale === "lab";
  if (domain === "otr") {
    const v = isLab ? (otr.otr_our_ratio_lab ?? 0) : (otr.otr_our_ratio_target ?? otr.kla_ratio);
    const s = isLab ? otr.otr_our_ratio_lab_std : otr.otr_our_ratio_target_std;
    return s ? `${fmt(v, 2)} ± ${fmt(s, 2)}` : fmt(v, 2);
  }
  if (domain === "mixing") {
    const v = isLab ? mixing.process_mixing_ratio_lab : mixing.process_mixing_ratio_target;
    const s = isLab ? mixing.process_mixing_ratio_lab_std : mixing.process_mixing_ratio_target_std;
    return `${fmt(v, 3)} ± ${fmt(s, 3)}`;
  }
  if (domain === "shear") {
    const v = isLab ? (shear.tip_speed_margin_lab ?? 0) : shear.tip_speed_margin;
    const s = isLab ? shear.tip_speed_margin_lab_std : shear.tip_speed_margin_std;
    return s ? `${fmt(v, 2)} ± ${fmt(s, 2)}` : fmt(v, 2);
  }
  if (domain === "co2") {
    const value = isLab ? co2.lab?.pco2_margin : co2.target?.pco2_margin ?? co2.pco2_margin;
    const s = isLab ? co2.lab?.pco2_margin_std : co2.target?.pco2_margin_std;
    if (!Number.isFinite(value ?? Infinity)) return "∞";
    return s ? `${fmt(value ?? 0, 2)} ± ${fmt(s, 2)}` : fmt(value ?? 0, 2);
  }
  const v = isLab ? (heat.lab?.heat_transfer_margin ?? 0) : (heat.target?.heat_transfer_margin ?? heat.heat_transfer_margin ?? 0);
  const s = isLab ? heat.lab?.heat_transfer_margin_std : heat.target?.heat_transfer_margin_std;
  return s ? `${fmt(v, 2)} ± ${fmt(s, 2)}` : fmt(v, 2);
}

function confidenceForDomain(domain: RiskDomain, results: PartialAssessmentResult): Confidence {
  if (domain === "otr") return results.otr.confidence;
  if (domain === "mixing") return results.mixing.confidence;
  if (domain === "shear") return results.shear.confidence;
  if (domain === "co2") return results.co2.confidence;
  return results.heat.confidence;
}

function calculatePdfPco2(scale: ReactorScaleConfig, inputs: ProcessInputs, derived: DerivedParameters): number | null {
  if (derived.our_peak <= CO2_OUR_THRESHOLD) return null;

  const cer = resolveRq(inputs.organism_species) * derived.our_peak;
  const klaCo2 = KLA_CO2_O2_RATIO * scale.kla_h;
  const vLiquidL = scale.geometry.volume_m3 * 1000;
  const qGasNlH = scale.gas.q_gas * 1000 * 3600;
  const nDotGasMol = qGasNlH / molarVolumeIdealGas(inputs.temperature);
  const cerMolH = (cer / 1000) * vLiquidL;
  const yCo2Out = Math.min(Y_CO2_INLET + cerMolH / Math.max(nDotGasMol, 1e-9), MAX_CO2_MOLE_FRACTION);
  const pco2GasIn  = Y_CO2_INLET * ATMOSPHERIC_PRESSURE_BAR;
  const pco2GasOut = yCo2Out     * ATMOSPHERIC_PRESSURE_BAR; // outlet at atmospheric — top of vessel
  const pco2GasAvg = Math.abs(pco2GasOut - pco2GasIn) < 1e-12
    ? pco2GasOut
    : (pco2GasOut - pco2GasIn) / Math.log(pco2GasOut / pco2GasIn);
  const pco2BulkAtm = (pco2GasAvg / ATMOSPHERIC_PRESSURE_BAR) + (cer / 1000) / (Math.max(klaCo2, 1e-9) * H_CO2);
  const pco2Bulk = pco2BulkAtm * ATMOSPHERIC_PRESSURE_BAR;
  const dpHydro = RHO * G * scale.geometry.h_liquid;
  return pco2Bulk * (ATMOSPHERIC_PRESSURE_PA + dpHydro) / ATMOSPHERIC_PRESSURE_PA;
}

function summarizePdfScale(scale: ReactorScaleConfig, inputs: ProcessInputs, derived: DerivedParameters): PdfProjectionScaleSummary {
  const heat = runHeatCapacityCheck({
    organism: inputs.organism_species,
    our_mmol_Lh: derived.our_peak,
    volume_litres: scale.volume_litres,
    t_process: inputs.temperature,
    t_cw_in: inputs.t_cw_inlet,
    flowrate_lpm: inputs.cooling_water_flowrate_lpm ?? 30,
    D_T: scale.geometry.t_diameter,
    H_L: scale.geometry.h_liquid,
    d_imp: scale.geometry.d_imp,
    N_rps: scale.rpm / 60,
    mu: derived.mu,
    impeller_type: inputs.impeller_type,
  });

  const mixingEns = computeMixingTimeEnsemble({
    T: scale.geometry.t_diameter,
    D: scale.geometry.d_imp,
    N_rps: scale.rpm / 60,
    pv_w_m3: scale.pv_w_m3,
    impeller_type: inputs.impeller_type,
  });

  return {
    volumeL: scale.volume_litres,
    rpm: scale.rpm,
    aerationLpm: scale.gas.q_gas * 1000 * 60,
    vvm: scale.vvm,
    impellerDiameterM: scale.geometry.d_imp,
    reactorHeightM: scale.geometry.h_liquid,
    klaMean: scale.kla_h,
    klaStd: scale.kla_ensemble.std,
    mixingTimeS: mixingEns.mean,
    mixingTimeStd: mixingEns.std,
    tipSpeedMS: scale.tip_speed_m_s,
    pco2Bar: calculatePdfPco2(scale, inputs, derived),
    metabolicHeatKW: heat.Q_metabolic_kW,
    reactorHeatCapacityKW: heat.Q_available_kW,
  };
}

function computePdfScaleProjection(inputs: ProcessInputs, derived: DerivedParameters): PdfScaleProjectionSummary {
  const method = inputs.scaleup_criterion ?? "power_per_volume";
  const pilotVolumeL = pilotVolume(inputs.v_lab, inputs.v_target);
  const base = buildReactorScaleConfigs(inputs, { method });
  const pilotInputs: ProcessInputs = { ...inputs, v_target: pilotVolumeL };
  const pilot = buildReactorScaleConfigs(pilotInputs, { method });
  return {
    lab: summarizePdfScale(base.lab, inputs, derived),
    pilot: summarizePdfScale(pilot.target, inputs, derived),
    production: summarizePdfScale(base.target, inputs, derived),
    pilotVolumeL,
  };
}

function scaleupWarnings(inputs: ProcessInputs): string[] {
  const method = inputs.scaleup_criterion ?? "power_per_volume";
  const configs = buildReactorScaleConfigs(inputs, { method });
  return [
    ...(configs.scaleup.clamped
      ? [`Target operating conditions were constrained, so target scale does not fully match the selected ${SCALEUP_CRITERION_LABELS[method]} criterion.`]
      : []),
    ...configs.scaleup.flags,
  ];
}

function PdfRadarChart({ title, scores }: { title: string; scores: Record<RiskDomain, RiskScore> }) {
  const size = 200;
  const cx = size / 2;
  const cy = size / 2;
  const maxR = 58;
  const labelR = 82;
  const points = DISPLAY_DOMAIN_ORDER.map((domain, index) => {
    const angle = -Math.PI / 2 + (index * 2 * Math.PI) / DISPLAY_DOMAIN_ORDER.length;
    return {
      domain,
      angle,
      x: cx + maxR * Math.cos(angle),
      y: cy + maxR * Math.sin(angle),
      lx: cx + labelR * Math.cos(angle),
      ly: cy + labelR * Math.sin(angle),
    };
  });
  const ring = (r: number) => points.map((p) => `${cx + maxR * r * Math.cos(p.angle)},${cy + maxR * r * Math.sin(p.angle)}`).join(" ");
  const dataPoints = points.map((p) => {
    const r = maxR * radarRadius(scores[p.domain]);
    return { domain: p.domain, x: cx + r * Math.cos(p.angle), y: cy + r * Math.sin(p.angle) };
  });
  const data = dataPoints.map((p) => `${p.x},${p.y}`).join(" ");
  const fillColor = pdfCompositeScoreColour(scores);

  return (
    <View style={{ width: "48%", alignItems: "center" }}>
      <Text style={[s.subsectionTitle, { marginTop: 0 }]}>{title}</Text>
      <Svg width={size} height={size}>
        {[0.25, 0.5, 0.75, 1].map((r) => (
          <Polygon
            key={r}
            points={ring(r)}
            fill={r === 1 ? "#F7FAFC" : "none"}
            stroke={r === 1 ? "#B8C7D6" : "#D4DEE8"}
            strokeWidth={r === 1 ? 1.1 : 0.9}
          />
        ))}
        {points.map((p) => (
          <Line key={p.domain} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="#C7D3DF" strokeWidth={0.8} />
        ))}
        <Polygon points={data} fill={fillColor} fillOpacity={0.24} stroke={fillColor} strokeWidth={1.5} />
        {dataPoints.map((p) => (
          <Circle key={p.domain} cx={p.x} cy={p.y} r={2.5} fill={RISK_COLOURS[scores[p.domain]]} />
        ))}
        {points.map((p) => {
          const anchor = Math.abs(p.lx - cx) < 5 ? "middle" : p.lx > cx ? "start" : "end";
          return (
            <Text
              key={`${p.domain}-name`}
              x={p.lx}
              y={p.ly - 3}
              textAnchor={anchor}
              style={{ fontSize: 6.5, fill: NAVY, fontFamily: "Helvetica-Bold" }}
            >
              {DOMAIN_SHORT_LABELS[p.domain]}
            </Text>
          );
        })}
        {points.map((p) => {
          const anchor = Math.abs(p.lx - cx) < 5 ? "middle" : p.lx > cx ? "start" : "end";
          return (
            <Text
              key={`${p.domain}-risk`}
              x={p.lx}
              y={p.ly + 5}
              textAnchor={anchor}
              style={{ fontSize: 6, fill: RISK_COLOURS[scores[p.domain]] }}
            >
              [{RISK_SCORE_LABELS[scores[p.domain]]}]
            </Text>
          );
        })}
      </Svg>
      <Text style={{ fontSize: 7, color: "#666666" }}>Outer ring: critical risk. Inner ring: low risk.</Text>
    </View>
  );
}

function PdfRiskSlider({ score, value }: { score: RiskScore; value: string }) {
  return (
    <View>
      <View style={{ flexDirection: "row", height: 8, marginBottom: 2 }}>
        {(["critical", "high", "moderate", "low"] as RiskScore[]).map((s0) => (
          <View key={s0} style={{
            flex: 1,
            backgroundColor: RISK_COLOURS[s0],
            opacity: s0 === score ? 1 : 0.35,
            borderWidth: s0 === score ? 1 : 0,
            borderColor: "#333333",
          }} />
        ))}
      </View>
      <Text style={{ fontSize: 7, color: "#555555" }}>Score {value} · {riskLabel(score)}</Text>
    </View>
  );
}

function PdfInfoBox({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <View style={{ backgroundColor: "#F0F4F8", borderRadius: 4, padding: 9, marginBottom: 10 }}>
      <Text style={[s.subsectionTitle, { marginTop: 0 }]}>{title}</Text>
      {children}
    </View>
  );
}

function InputTable({ title, rows }: { title: string; rows: { label: string; value: string }[] }) {
  return (
    <View style={{ width: "48%", marginBottom: 10 }}>
      <Text style={s.subsectionTitle}>{title}</Text>
      <View style={s.paramTable}>
        {rows.map((row, index) => (
          <View key={index} style={s.paramTableRow}>
            <Text style={s.paramTableLabel}>{row.label}</Text>
            <Text style={s.paramTableValue}>{row.value}</Text>
          </View>
        ))}
      </View>
    </View>
  );
}

function CalculationRow({ label, equation, value, finalValue }: { label: string; equation?: string; value?: string; finalValue: string }) {
  return (
    <View style={{ borderBottomWidth: 0.5, borderBottomColor: "#E8EDF2", paddingVertical: 4 }}>
      <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", color: NAVY, marginBottom: 2 }}>{label}</Text>
      {equation ? <Text style={{ fontSize: 7, color: "#555555" }}>Equation: {equation}</Text> : null}
      {value ? <Text style={{ fontSize: 7, color: "#555555" }}>Value: {value}</Text> : null}
      <Text style={{ fontSize: 8, fontFamily: "Courier", color: "#222222" }}>Final: {finalValue}</Text>
    </View>
  );
}

function BaselineExecutiveSummary({ inputs, derived, results }: PdfReportProps) {
  const species = SPECIES_LABELS[inputs.organism_species] ?? inputs.organism_species;
  const scaleRatio = inputs.v_target / inputs.v_lab;
  const warnings = scaleupWarnings(inputs);
  const method = inputs.scaleup_criterion ?? "power_per_volume";
  const configs = buildReactorScaleConfigs(inputs, { method });

  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Executive Summary</Text>
      <View style={s.paramBox}>
        <View style={s.paramRow}><Text style={s.paramLabel}>Organism</Text><Text style={s.paramValue}>{species}</Text></View>
        <View style={s.paramRow}><Text style={s.paramLabel}>Scale-up</Text><Text style={s.paramValue}>{fmtInt(inputs.v_lab)} L to {fmtInt(inputs.v_target)} L ({fmt(scaleRatio, 0)}x)</Text></View>
        <View style={s.paramRow}><Text style={s.paramLabel}>Criterion</Text><Text style={s.paramValue}>{SCALEUP_CRITERION_LABELS[method]}</Text></View>
        <View style={s.paramRow}><Text style={s.paramLabel}>Peak OUR</Text><Text style={s.paramValue}>{fmt(derived.our_peak, 1)} mmol/L/h</Text></View>
      </View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
        <PdfRadarChart title="Lab Scale Risk Profile" scores={labScores(results)} />
        <PdfRadarChart title="Target Scale Risk Profile" scores={targetScores(results)} />
      </View>
      <View style={{ flexDirection: "row", justifyContent: "center", marginBottom: 10 }}>
        {(["low", "moderate", "high", "critical"] as RiskScore[]).map((r) => (
          <View key={r} style={{ flexDirection: "row", alignItems: "center", marginRight: 14 }}>
            <View style={{ width: 7, height: 7, borderRadius: 4, backgroundColor: RISK_COLOURS[r], marginRight: 3 }} />
            <Text style={{ fontSize: 7, color: "#555555" }}>{RISK_SCORE_LABELS[r]}</Text>
          </View>
        ))}
      </View>
      <PdfInfoBox title="Primary Bottleneck">
        <Text style={s.bodyText}>{results.primary_bottleneck.statement}</Text>
      </PdfInfoBox>
      <PdfInfoBox title="Scale-Up Constraints">
        <Text style={s.bodyText}>
          Scale-up performed according to {SCALEUP_CRITERION_LABELS[method]} criterion. Impeller at target scale set to run at {fmt(configs.target.rpm, 0)} RPM and aeration rate at target scale set to {fmt(configs.target.vvm, 2)} vvm.
        </Text>
        {warnings.map((warning, index) => (
          <Text key={index} style={[s.bodyText, { color: "#8A5A00" }]}>Warning: {warning}</Text>
        ))}
      </PdfInfoBox>
      <PageFooter />
    </Page>
  );
}

function BaselineInputSummary({ inputs, derived }: PdfReportProps) {
  const species = SPECIES_LABELS[inputs.organism_species] ?? inputs.organism_species;
  const method = inputs.scaleup_criterion ?? "power_per_volume";
  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Input Summary</Text>
      <View style={{ flexDirection: "row", justifyContent: "space-between", flexWrap: "wrap" }}>
        <InputTable title="Organism Identity and Process Type" rows={[
          { label: "Organism", value: species },
          { label: "Process type", value: inputs.process_type === "fed_batch" ? "Fed-batch" : "Batch" },
          ...(inputs.process_type === "fed_batch" && inputs.feeding_frequency
            ? [{ label: "Feeding frequency", value: FEEDING_FREQUENCY_LABELS[inputs.feeding_frequency] }]
            : []),
          { label: "Biomass", value: `${fmt(inputs.biomass_cdw_g_l, 1)} g/L CDW` },
          { label: "OUR mode", value: inputs.our_mode === "measured" ? "Measured" : "Estimated" },
          { label: "OUR", value: `${fmt(derived.our_peak, 1)} mmol/L/h` },
          { label: "Temperature", value: `${fmt(inputs.temperature, 1)} °C` },
          { label: "DO set point", value: `${fmt(inputs.do_setpoint, 1)} %` },
          { label: "O2 inlet", value: `${fmt(inputs.o2_inlet ?? 20.9, 1)} %` },
        ]} />
        <InputTable title="Lab Scale Reactor" rows={[
          { label: "Volume", value: `${fmtInt(inputs.v_lab)} L` },
          { label: "Impeller", value: inputs.impeller_type.replace(/_/g, " ") },
          { label: "RPM", value: `${fmt(inputs.rpm, 0)} rpm` },
          { label: "Aeration", value: `${fmt(inputs.vvm, 2)} vvm` },
          { label: "H/T", value: fmt(inputs.h_d_lab, 2) },
          { label: "D/T", value: fmt(inputs.dt_ratio_lab ?? 0, 2) },
          { label: "Impellers", value: fmtInt(inputs.n_impellers) },
        ]} />
        <InputTable title="Target Scale Design" rows={[
          { label: "Volume", value: `${fmtInt(inputs.v_target)} L` },
          { label: "Criterion", value: SCALEUP_CRITERION_LABELS[method] },
          { label: "H/T", value: fmt(inputs.h_d_target, 2) },
          { label: "D/T", value: fmt(inputs.dt_ratio_target ?? 0, 2) },
          { label: "Impellers", value: fmtInt(inputs.n_impellers_target ?? inputs.n_impellers) },
        ]} />
        <InputTable title="Cooling Inputs" rows={[
          { label: "Cooling utility inlet", value: `${fmt(inputs.t_cw_inlet, 1)} °C` },
          { label: "Cooling flowrate (assumed)", value: `${fmt(inputs.cooling_water_flowrate_lpm ?? 30, 0)} L/min` },
        ]} />
      </View>
      <PageFooter />
    </Page>
  );
}

function BaselineRiskOverview({ results }: PdfReportProps) {
  const lab = labScores(results);
  const target = targetScores(results);
  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Risk Overview</Text>
      {DISPLAY_DOMAIN_ORDER.map((domain) => (
        <View key={domain} style={{ marginBottom: 11, paddingBottom: 8, borderBottomWidth: 0.5, borderBottomColor: "#E0E0E0" }} wrap={false}>
          <View style={s.domainHeader}>
            <Text style={s.domainName}>{DOMAIN_FULL_LABELS[domain]}</Text>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: CONFIDENCE_DOT[confidenceForDomain(domain, results)], marginRight: 4 }} />
              <Text style={{ fontSize: 7, color: "#666666" }}>{confidenceLabel(confidenceForDomain(domain, results))}</Text>
            </View>
          </View>
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <View style={{ width: "48%" }}>
              <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", marginBottom: 3 }}>Lab scale</Text>
              <PdfRiskSlider score={lab[domain]} value={scoreValue(domain, results, "lab")} />
            </View>
            <View style={{ width: "48%" }}>
              <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", marginBottom: 3 }}>Target scale</Text>
              <PdfRiskSlider score={target[domain]} value={scoreValue(domain, results, "target")} />
            </View>
          </View>
        </View>
      ))}
      <PageFooter />
    </Page>
  );
}

function calcLines(domain: RiskDomain, scale: "lab" | "target", inputs: ProcessInputs, derived: DerivedParameters, results: PartialAssessmentResult): { label: string; equation?: string; value?: string; finalValue: string }[] {
  const { otr, mixing, shear, co2, heat } = results;
  const isLab = scale === "lab";
  if (domain === "otr") {
    const capacity = isLab ? (otr.otr_capacity_lab ?? 0) : (otr.otr_capacity_target ?? 0);
    const kla = isLab ? otr.kla_lab : otr.kla_target_moderate;
    const klaStd = isLab ? otr.kla_lab_std : otr.kla_target_std;
    const dc = isLab ? derived.df_lm_lab : derived.driving_force;
    const ratio = derived.our_peak > 0 ? capacity / derived.our_peak : 0;
    const ratioStd = isLab ? otr.otr_our_ratio_lab_std : otr.otr_our_ratio_target_std;
    return [
      { label: "OUR", finalValue: `${fmt(derived.our_peak, 1)} mmol/L/h` },
      { label: "kLa", finalValue: klaStd ? `${fmt(kla, 1)} ± ${fmt(klaStd, 1)} h^-1` : `${fmt(kla, 1)} h^-1` },
      { label: "OTR", equation: "OTR = kLa x Delta C_LM", value: `${fmt(kla, 1)} x ${fmt(dc, 3)}`, finalValue: `${fmt(capacity, 1)} mmol/L/h` },
      { label: "Score", equation: "Score = OTR / OUR", value: `${fmt(capacity, 1)} / ${fmt(derived.our_peak, 1)}`, finalValue: ratioStd ? `${fmt(ratio, 2)} ± ${fmt(ratioStd, 2)}` : fmt(ratio, 2) },
    ];
  }
  if (domain === "mixing") {
    const cStar = isLab ? derived.c_star_lab : derived.c_star;
    const theta = isLab ? mixing.theta_mix_lab : mixing.theta_mix_target;
    const thetaStd = isLab ? mixing.theta_mix_lab_std : mixing.theta_mix_target_std;
    const tauO2 = isLab ? mixing.oxygen_depletion_time_lab_s : mixing.oxygen_depletion_time_target_s;
    const tProcess = isLab ? mixing.t_process_lab_s : mixing.t_process_target_s;
    const ratio = isLab ? mixing.process_mixing_ratio_lab : mixing.process_mixing_ratio_target;
    const ratioStd = isLab ? mixing.process_mixing_ratio_lab_std : mixing.process_mixing_ratio_target_std;
    const geom = isLab ? derived.lab_geometry : derived.target_geometry;
    const isFedBatch = inputs.process_type === "fed_batch" && mixing.t_feed_s !== undefined;
    const feedRows = isFedBatch ? [
      { label: "tau_feed", equation: "tau_feed = feed pulse interval",
        value: FEEDING_FREQUENCY_LABELS[(inputs.feeding_frequency ?? "10_30min") as FeedingFrequency],
        finalValue: `${fmt(mixing.t_feed_s ?? 0, 0)} s` },
      { label: "tau_process", equation: "tau_process = min(tau_O2, tau_feed)",
        value: `min(${fmt(tauO2, 1)}, ${fmt(mixing.t_feed_s ?? 0, 0)})`,
        finalValue: `${fmt(tProcess, 1)} s` },
    ] : [];
    return [
      { label: "tau_O2", equation: "tau_O2 = 3600 x C* / OUR", value: `3600 x ${fmt(cStar, 3)} / ${fmt(derived.our_peak, 1)}`, finalValue: `${fmt(tauO2, 2)} s` },
      ...feedRows,
      { label: "tau_mix", equation: "Grenville-Nienow: tau_mix = C_GN * T^(2/3) * epsilon^(-1/3) * (D/T)^(-1/3) / N", value: `T=${fmt(geom.t_diameter, 3)} m, d_i=${fmt(geom.d_imp, 3)} m`, finalValue: `${fmt(theta, 2)} ± ${fmt(thetaStd, 2)} s` },
      { label: "Score", equation: "Score = tau_process / tau_mix", value: `${fmt(tProcess, 1)} / ${fmt(theta, 2)}`, finalValue: `${fmt(ratio, 3)} ± ${fmt(ratioStd, 3)}` },
    ];
  }
  if (domain === "shear") {
    const tip = isLab ? (shear.tip_speed_lab ?? 0) : shear.tip_speed;
    const n = isLab ? (shear.n_lab ?? 0) : shear.n_target;
    const d = isLab ? derived.lab_geometry.d_imp : derived.target_geometry.d_imp;
    const margin = isLab ? (shear.tip_speed_margin_lab ?? 0) : shear.tip_speed_margin;
    const marginStd = isLab ? shear.tip_speed_margin_lab_std : shear.tip_speed_margin_std;
    return [
      { label: "v_tip", equation: "v_tip = pi N d_i", value: `pi x ${fmt(n, 2)} x ${fmt(d, 3)}`, finalValue: `${fmt(tip, 2)} m/s` },
      { label: "Threshold", finalValue: `${fmt(shear.tip_speed_threshold, 2)} m/s` },
      { label: "Score", equation: "Score = v_tip threshold / v_tip impeller", value: `${fmt(shear.tip_speed_threshold, 2)} / ${fmt(tip, 2)}`, finalValue: marginStd ? `${fmt(margin, 2)} ± ${fmt(marginStd, 2)}` : fmt(margin, 2) },
    ];
  }
  if (domain === "co2") {
    const scaleResult = isLab ? co2.lab : co2.target;
    if (!co2.activated || !scaleResult) {
      return [
        { label: "Status", finalValue: "Inactive below biomass/OUR trigger thresholds" },
        { label: "Score", finalValue: "∞" },
      ];
    }
    return [
      { label: "CER", equation: "CER = RQ x OUR", value: `${fmt(resolveRq(inputs.organism_species), 2)} x ${fmt(derived.our_peak, 1)}`, finalValue: `${fmt(scaleResult.cer, 1)} mmol/L/h` },
      { label: "kLa_CO2", equation: "kLa_CO2 = kLa_O2 x sqrt(D_CO2/D_O2)", finalValue: `${fmt(scaleResult.kla_co2, 1)} h^-1` },
      { label: "Score", equation: "Score = P_CO2 threshold / P_CO2 bottom", value: `${fmt(co2.pco2_critical ?? 0, 3)} / ${fmt(scaleResult.pco2_bottom, 3)}`, finalValue: scaleResult.pco2_margin_std ? `${fmt(scaleResult.pco2_margin, 2)} ± ${fmt(scaleResult.pco2_margin_std, 2)}` : fmt(scaleResult.pco2_margin, 2) },
    ];
  }
  const heatScale = isLab ? heat.lab : heat.target;
  const qCool = heatScale?.q_cool_max ?? heat.q_cool_max;
  const qMet = heatScale?.q_metabolic ?? heat.q_metabolic;
  const u = heatScale?.u_overall ?? heat.u_overall ?? 0;
  const area = heatScale?.a_jacket ?? heat.a_jacket;
  const dt = heatScale?.dt_lm ?? heat.dt_lm;
  const heatMargin = qMet > 0 ? qCool / qMet : 0;
  const heatMarginStd = heatScale?.heat_transfer_margin_std;
  return [
    { label: "Q_cooling", equation: "Q_cooling = U A Delta T_LM", value: `${fmt(u, 1)} x ${fmt(area, 2)} x ${fmt(dt, 1)} / 1000`, finalValue: `${fmt(qCool, 2)} kW` },
    { label: "Q_metabolic", equation: "Q_metabolic = Delta H_metabolic x OUR x V / 3600", value: `${fmt(METABOLIC_HEAT_COEFF_KJ_PER_MMOL[inputs.organism_species], 2)} x ${fmt(derived.our_peak, 1)} x V / 3600`, finalValue: `${fmt(qMet, 2)} kW` },
    { label: "Score", equation: "Score = Qcooling / Qmetabolic", value: `${fmt(qCool, 2)} / ${fmt(qMet, 2)}`, finalValue: heatMarginStd ? `${fmt(heatMargin, 2)} ± ${fmt(heatMarginStd, 2)}` : fmt(heatMargin, 2) },
  ];
}

// ARCHIVED — Parameter Estimation Notes PDF page. To restore: uncomment in PdfReportDocument.
function ParameterEstimationNotes({ inputs, derived, results }: PdfReportProps) {
  const ourSource = inputs.our_mode === "measured"
    ? "OUR was supplied by the user as a measured value."
    : `OUR was estimated from literature ranges for the selected organism at ${fmt(inputs.biomass_cdw_g_l, 1)} g/L CDW (${fmt(derived.our_min ?? derived.our_peak, 1)}–${fmt(derived.our_max ?? derived.our_peak, 1)} mmol/L/h).`;
  const notes = [
    ourSource,
    "kLa values are calculated with an ensemble of stirred-tank correlations selected from the local correlation bank, including Van’t Riet, Garcia-Ochoa and Gomez, Linek et al., and impeller-specific forms where applicable.",
    "Mixing time is estimated as the ensemble mean of Grenville-Nienow (2002) and Ruszkowski (1994) correlations, using reactor geometry, impeller diameter, and turbulent energy dissipation.",
    "Shear risk uses organism-specific tip-speed thresholds and the impeller tip speed calculated from RPM and impeller diameter.",
    "CO2 accumulation uses a simplified mass balance with organism respiratory quotient defaults, gas flow, hydrostatic pressure, and kLa_CO2 estimated from the O2 kLa diffusivity ratio.",
    "Heat removal uses metabolic heat factor, jacket area, LMTD, and an overall U assembled from broth-side, wall, fouling, and jacket-side thermal resistances.",
  ];
  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Parameter Estimation Notes</Text>
      {notes.map((note, index) => (
        <View key={index} style={{ flexDirection: "row", marginBottom: 6 }}>
          <Text style={{ fontSize: 8, color: TEAL, marginRight: 5 }}>{index + 1}.</Text>
          <Text style={[s.bodyText, { flex: 1, marginBottom: 0 }]}>{note}</Text>
        </View>
      ))}
      <PageFooter />
    </Page>
  );
}

// ARCHIVED — Calculation Breakdown PDF page. Not rendered; kept for reference.
function BaselineCalculationBreakdown({ inputs, derived, results }: PdfReportProps) {
  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Calculation Breakdown</Text>
      {DISPLAY_DOMAIN_ORDER.map((domain) => (
        <View key={domain} style={s.domainSection} wrap={false}>
          <Text style={s.domainName}>{DOMAIN_FULL_LABELS[domain]}</Text>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 4 }}>
            <View style={{ width: "48%" }}>
              <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", color: TEAL }}>Lab scale</Text>
              {calcLines(domain, "lab", inputs, derived, results).map((line, index) => <CalculationRow key={index} {...line} />)}
            </View>
            <View style={{ width: "48%" }}>
              <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", color: TEAL }}>Target scale</Text>
              {calcLines(domain, "target", inputs, derived, results).map((line, index) => <CalculationRow key={index} {...line} />)}
            </View>
          </View>
        </View>
      ))}
      <PageFooter />
    </Page>
  );
}

function BaselineProjections({ inputs, derived }: PdfReportProps) {
  const projection = computePdfScaleProjection(inputs, derived);
  const rows = [
    { param: "Impeller RPM (rpm)", lab: fmt(projection.lab.rpm, 0), pilot: fmt(projection.pilot.rpm, 0), prod: fmt(projection.production.rpm, 0) },
    { param: "Aeration rate (L/min, vvm)", lab: `${fmt(projection.lab.aerationLpm, 2)} (${fmt(projection.lab.vvm, 2)})`, pilot: `${fmt(projection.pilot.aerationLpm, 2)} (${fmt(projection.pilot.vvm, 2)})`, prod: `${fmt(projection.production.aerationLpm, 2)} (${fmt(projection.production.vvm, 2)})` },
    { param: "Impeller diameter (m)", lab: fmt(projection.lab.impellerDiameterM, 2), pilot: fmt(projection.pilot.impellerDiameterM, 2), prod: fmt(projection.production.impellerDiameterM, 2) },
    { param: "Reactor height (m)", lab: fmt(projection.lab.reactorHeightM, 2), pilot: fmt(projection.pilot.reactorHeightM, 2), prod: fmt(projection.production.reactorHeightM, 2) },
    { param: "kLa (h^-1)", lab: fmtRange95(projection.lab.klaMean, projection.lab.klaStd), pilot: fmtRange95(projection.pilot.klaMean, projection.pilot.klaStd), prod: fmtRange95(projection.production.klaMean, projection.production.klaStd) },
    { param: "Mixing time (s)", lab: fmtRange95(projection.lab.mixingTimeS, projection.lab.mixingTimeStd), pilot: fmtRange95(projection.pilot.mixingTimeS, projection.pilot.mixingTimeStd), prod: fmtRange95(projection.production.mixingTimeS, projection.production.mixingTimeStd) },
    { param: "Tip speed (m/s)", lab: fmt(projection.lab.tipSpeedMS, 2), pilot: fmt(projection.pilot.tipSpeedMS, 2), prod: fmt(projection.production.tipSpeedMS, 2) },
    { param: "P_CO2 (bar)", lab: projection.lab.pco2Bar == null ? "—" : fmt(projection.lab.pco2Bar, 2), pilot: projection.pilot.pco2Bar == null ? "—" : fmt(projection.pilot.pco2Bar, 2), prod: projection.production.pco2Bar == null ? "—" : fmt(projection.production.pco2Bar, 2) },
    { param: "Metabolic heat (kW)", lab: fmt(projection.lab.metabolicHeatKW, 2), pilot: fmt(projection.pilot.metabolicHeatKW, 2), prod: fmt(projection.production.metabolicHeatKW, 2) },
    { param: "Cooling capacity (kW)", lab: fmt(projection.lab.reactorHeatCapacityKW, 2), pilot: fmt(projection.pilot.reactorHeatCapacityKW, 2), prod: fmt(projection.production.reactorHeatCapacityKW, 2) },
  ];
  return (
    <Page size="A4" style={s.page}>
      <PageHeader />
      <Text style={s.sectionTitle}>Scale-Up Projections</Text>
      <Text style={s.bodyText}>Baseline projections across lab, pilot ({fmtInt(projection.pilotVolumeL)} L), and production scale. Reactor What-If modifications are not included.</Text>
      <View style={s.projTable}>
        <View style={s.projHeader}>
          <Text style={[s.projHeaderCell, s.projParamCol]}>Parameter</Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>Lab ({fmtInt(inputs.v_lab)} L)</Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>Pilot ({fmtInt(projection.pilotVolumeL)} L)</Text>
          <Text style={[s.projHeaderCell, s.projValCol]}>Production ({fmtInt(inputs.v_target)} L)</Text>
        </View>
        {rows.map((row, index) => (
          <View key={index} style={index % 2 === 0 ? s.projRow : s.projRowAlt}>
            <Text style={[{ fontSize: 7 }, s.projParamCol]}>{row.param}</Text>
            <Text style={[{ fontSize: 7, fontFamily: "Courier" }, s.projValCol]}>{row.lab}</Text>
            <Text style={[{ fontSize: 7, fontFamily: "Courier" }, s.projValCol]}>{row.pilot}</Text>
            <Text style={[{ fontSize: 7, fontFamily: "Courier" }, s.projValCol]}>{row.prod}</Text>
          </View>
        ))}
      </View>
      <View style={s.scopeBox}>
        <Text style={[s.subsectionTitle, { color: NAVY, marginTop: 0 }]}>Scope Limitation</Text>
        <Text style={s.scopeText}>
          This assessment is based on empirical correlations with ±30–40% uncertainty on kLa. It does not predict yield or replace pilot experimentation.
        </Text>
      </View>
      <PageFooter />
    </Page>
  );
}

// --- Main document ---

export interface PdfReportProps {
  inputs: ProcessInputs;
  derived: DerivedParameters;
  results: PartialAssessmentResult;
}

export function PdfReportDocument({ inputs, derived, results }: PdfReportProps) {
  return (
    <Document
      title="Lemnisca Scale-Up Risk Assessment"
      author="Lemnisca Fermentation Scale-Up Risk Predictor"
    >
      <BaselineExecutiveSummary inputs={inputs} derived={derived} results={results} />
      <BaselineInputSummary inputs={inputs} derived={derived} results={results} />
      <BaselineRiskOverview inputs={inputs} derived={derived} results={results} />
      {/* ARCHIVED — Calculation Breakdown page. To restore: uncomment the line below.
      <BaselineCalculationBreakdown inputs={inputs} derived={derived} results={results} />
      */}
      <BaselineProjections inputs={inputs} derived={derived} results={results} />
      {/* ARCHIVED — Parameter Estimation Notes page. To restore: uncomment the line below.
      <ParameterEstimationNotes inputs={inputs} derived={derived} results={results} />
      */}
    </Document>
  );
}
