...
import type { ProcessInputs, VesselGeometry } from "../types";
import { HIGH_DENSITY_BIOMASS_G_L } from "../constants";
import type {
  ScaleupCriterion,
...
function buildScaleupInput(
  inputs: ProcessInputs,
  labGeometry: VesselGeometry,
  targetGeometry: VesselGeometry,
): ScaleupCriteriaInput {
  return {
    v_lab:              inputs.v_lab,
    ...
    n_impellers_target: inputs.n_impellers_target ?? inputs.n_impellers,
    mu:                 deriveViscosity(inputs.temperature),
    is_high_density:    inputs.biomass_cdw_g_l >= HIGH_DENSITY_BIOMASS_G_L,
    lab_geometry:       labGeometry,
...
function buildScaleConfig(params: {
  ...
  return {
    ...
    kla_h:         params.kla_h,
    kla_ensemble:  params.kla_ensemble,
...
export function buildReactorScaleConfigs(
  ...
  const scaleupInput = buildScaleupInput(inputs, labGeometry, targetGeometry);
  ...
  let targetKlaEnsemble = scaleup.target_kla_ensemble;
  ...
  if (inputs.target_rpm_override !== undefined) {
    ...
    const is_high_density = inputs.biomass_cdw_g_l >= HIGH_DENSITY_BIOMASS_G_L;
    ...
    targetKlaEnsemble = computeKlaEnsemble(op, targetPowerW, is_high_density);
    targetKlaH        = targetKlaEnsemble.mean;
  ...
  return {
    lab: buildScaleConfig({
      ...
      kla_ensemble:  scaleup.lab_kla_ensemble,
    ...
    target: buildScaleConfig({
      ...
      pv_w_m3:       targetPvWM3,
      kla_h:         targetKlaH,
      kla_ensemble:  targetKlaEnsemble,
