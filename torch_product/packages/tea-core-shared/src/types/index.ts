...
// --- Engine response shapes (structural — no math, no IP) -----------------
// Describe the shape of the data returned by the backend's assessment
// endpoints so the frontend can type-check API responses. The actual math
// lives in @torch/core (backend-only).
...
export interface PartialAssessmentResult {
  ...
  flags: AssessmentFlag[];
  derived: DerivedParameters;
  /** Scale-specific operating configurations (lab + target + scale-up criteria). */
  reactor_configs?: ReactorScaleConfigs;
}
