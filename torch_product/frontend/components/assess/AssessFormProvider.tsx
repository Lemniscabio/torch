...
export function AssessFormProvider({ children }: { children: React.ReactNode }) {
  ...
  const methods = useForm<AssessFormValues>({
    ...
    delayError: 180,
    defaultValues: ASSESS_DEFAULTS as AssessFormValues,
  ...
  useEffect(() => {
    ...
    const cleanDraft = draft
      ...
      : {};
    methods.reset({ ...ASSESS_DEFAULTS, ...cleanDraft } as AssessFormValues);
    setDraftReady(true);
  }, [methods]);

  return (
    <FormProvider {...methods}>
      {draftReady ? <DraftAutosave methods={methods} /> : null}
      {children}
    </FormProvider>
  );
}
